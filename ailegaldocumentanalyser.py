# -*- coding: utf-8 -*-
"""AiLegalDocumentAnalyser.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/18_0r595UAjgu0-5jvQlamZ2XhVKigurK
"""

from google.colab import userdata
userdata.get('ailegal')

!pip install spacy nltk gradio
!python -m spacy download en_core_web_sm
import nltk
nltk.download('punkt') # For sentence splitting
nltk.download('wordnet') # For finding synonyms
...
import gradio as gr
import re # For simple splitting rules
...
def simplify_lexically(text, changes_log):
    """
    Identifies and replaces words from LEGAL_JARGON_MAP.
    Logs changes made.
    """
    simplified_text = text
    words_replaced = []

    # Use regex to find whole words to avoid replacing parts of words
    for jargon, simple_term in LEGAL_JARGON_MAP.items():
        # \b ensures we match whole words only
        pattern = r'\b' + re.escape(jargon) + r'\b'
        # Perform case-insensitive replacement
        new_text, num_replacements = re.subn(pattern, simple_term, simplified_text, flags=re.IGNORECASE)

        if num_replacements > 0:
            simplified_text = new_text
            # Log the change only once per jargon term found
            if jargon not in [item['original'] for item in words_replaced]:
                 words_replaced.append({'type': 'lexical', 'original': jargon, 'simplified': simple_term, 'reason': 'Replaced complex legal term.'})

    # Add all logged changes for this step
    changes_log.extend(words_replaced)
    return simplified_text

# --- Test the function ---
test_text_lexical = "Pursuant to the aforementioned agreement, the parties shall commence work."
log_lexical = []
simplified_lexical = simplify_lexically(test_text_lexical, log_lexical)
print("Original:", test_text_lexical)
print("Simplified:", simplified_lexical)
print("Changes:", log_lexical)

# Syntactic Simplification Function
def simplify_syntactically(text, changes_log, potential_split_conjunctions=['and', 'or', 'but', 'whereas', ';', 'however']):
    """
    Processes EVERY sentence. Flags potential split points or notes simplicity.
    Logs analysis for each sentence.
    """
    doc = nlp(text) # Process text with spaCy
    simplified_sentences = [] # We will build the output sentence by sentence
    original_sentences = [] # Keep track of original sentences

    print(f"\nProcessing {len(list(doc.sents))} sentences syntactically...") # Debug print

    for i, sent in enumerate(doc.sents):
        original_sentences.append(sent.text)
        words = [token.text for token in sent if not token.is_punct]
        word_count = len(words)
        found_potential_split = False
        split_candidates = []

        # Check for potential split points (simple conjunction check)
        for token in sent:
            if token.text.lower() in potential_split_conjunctions:
                 # Basic check: avoid splitting if conjunction is near start/end
                 if token.i > 2 and token.i < len(sent) - 2:
                    found_potential_split = True
                    split_candidates.append(token.text)

        # Log analysis for EVERY sentence
        if found_potential_split:
            reason = f"Contains potential split point(s): {', '.join(set(split_candidates))}."
            simplified_version_info = f"[Sentence {i+1} processed. It could potentially be split.]"
            changes_log.append({
                'type': 'syntactic_potential_split',
                'original': sent.text,
                'simplified': simplified_version_info,
                'reason': reason
            })
        elif word_count > 30 : # Keep the length flag for very long ones
             reason = f"Sentence is long ({word_count} words)."
             simplified_version_info = f"[Sentence {i+1} processed. It is quite long.]"
             changes_log.append({
                'type': 'syntactic_long',
                'original': sent.text,
                'simplified': simplified_version_info,
            ...
            # Sentence is short and has no obvious split points based on simple rules
            reason = f"Sentence is relatively simple ({word_count} words)."
            simplified_version_info = f"[Sentence {i+1} processed. Appears syntactically simple.]"
            changes_log.append({
                'type': 'syntactic_simple',
                'original': sent.text,
                'simplified': simplified_version_info, # No change needed, just logging analysis
                'reason': reason
            })

        # Add the original sentence text back for now (no actual splitting)
        simplified_sentences.append(sent.text)

    # Return the text structurally unchanged, along with original sentences list
    return " ".join(simplified_sentences), original_sentences

# --- Test the REVISED function ---
test_text_syntactic = "This agreement, including all exhibits and attachments thereto, represents the entire understanding between the parties concerning the subject matter herein; it supersedes all prior discussions, negotiations, and agreements, whether oral or written, relating thereto, requiring careful consideration by all involved. Shorter sentences are also processed."
log_syntactic = []
simplified_syntactic, original_syntactic = simplify_syntactically(test_text_syntactic, log_syntactic)
print("\nOriginal:", test_text_syntactic)
print("\nSimplified Text (Structure Unchanged):", simplified_syntactic)
print("\nChanges Log:", log_syntactic) # Log will show analysis for ALL sentences

# Main Processing Function
def simplify_and_explain(legal_text):
    """
    Runs the simplification pipeline (lexical changes applied, syntactic changes logged)
    and returns results for Gradio.
    """
    if not legal_text or not legal_text.strip():
        return "", "", "Please enter some text."

    changes_log = [] # Initialize log for this run

    # 1. Lexical Simplification (Applied)
    lexically_simplified_text = simplify_lexically(legal_text, changes_log)
    print("\n--- Lexical Simplification Done ---")


    # 2. Syntactic Simplification (Analysis Logged, Structure Unchanged)
    # We pass the *lexically simplified* text to the syntactic analyzer
    final_text_structure_unchanged, original_sentences = simplify_syntactically(lexically_simplified_text, changes_log)
    print("\n--- Syntactic Analysis Done ---")

    # 3. Format the change log for display
    explanation_output = "Simplification Details:\n" + "="*20 + "\n"
    if not changes_log:
        explanation_output += "No specific simplifications applied or analysis logged based on current rules.\n"
    else:
        lex_changes_count = 0
        syn_changes_count = 0
        for change in changes_log:
            if change['type'] == 'lexical':
                explanation_output += f"- LEXICAL: Replaced '{change['original']}' with '{change['simplified']}'. Reason: {change['reason']}\n"
                lex_changes_count += 1
            elif change['type'].startswith('syntactic'):
                # Show analysis for every sentence processed syntactically
                 explanation_output += f"- SYNTACTIC Analysis: Sentence starting '{change['original'][:50]}...' -> {change['reason']}\n"
                 syn_changes_count += 1

        if lex_changes_count == 0:
             explanation_output += "\nNo lexical changes applied.\n"
        if syn_changes_count == 0:
             explanation_output += "\nNo syntactic analysis performed (or text was empty).\n"


    # Return original, the lexically simplified text, and the explanation log
    # The simplified text output ONLY contains the word replacements, not structural changes yet.
    return legal_text, lexically_simplified_text, explanation_output

# --- Test the REVISED main function ---
test_text_main = "Whereas, pursuant to the aforementioned agreement, the parties shall commence work heretofore; this agreement shall terminate upon completion. This complex part is designed to test sentence analysis but is shorter."
original, simplified, explanation = simplify_and_explain(test_text_main)
print("\n--- REVISED Main Function Test ---")
print("Original Input:\n", original)
print("\nSimplified Output (Lexical Only):\n", simplified) # Note: Structure is same as original
print("\nExplanation Log:\n", explanation)

# === Code Cell 7: Gradio Interface ===
import gradio as gr

# Define the interface using the main function
iface = gr.Interface(
    fn=simplify_and_explain, # The Python function to call
    inputs=gr.Textbox(lines=20, label="Paste Legal Text Here"), # Input component
    outputs=[
        gr.Textbox(label="Original Text"), # Output component 1
        gr.Textbox(label="Simplified Text"), # Output component 2
        gr.Textbox(label="Explanation of Changes") # Output component 3
    ],
    title="PlainSpeak Legal Analyzer",
    description="Enter a snippet of legal text (e.g., a clause from Terms of Service). The tool will replace some complex legal jargon and flag long sentences. Explanations for changes are shown below.",
    allow_flagging='never' # Disables user feedback buttons for simplicity
)
